...
# \todo Might want to make sure that a sprite's CHR data never gets divided
#       into multiple banks (or have an option for that).
...
def writeData( outPrefix, allAnimationFrames, tiles, hardwareSpriteSize,
    ...
    with open( outPrefix + ".s", "w" ) as f:
        ...
        bytesLeft = len( tiles )*(
            kBytesPer8x8Tile if hardwareSpriteSize == HardwareSpriteSize.k8x8 \
            else kBytesPer8x16Tile
        )
        ...
        for i in range( len( segments ) ):
            if hardwareSpriteSize == HardwareSpriteSize.k8x16:
                f.write( "B_{} = .lobyte( chrData_{}/ppu::kBytesPer8x16Tile )\n".format( i, i ) )
            else:
        ...
        for symbol, animationFrames in allAnimationFrames:
            for frameIndex, frame in enumerate( animationFrames ):
                ...
                for sprite in frame.sprites:
                    tile = None
                    if hardwareSpriteSize == HardwareSpriteSize.k8x16:
                        tile = "_8x16Tile B_{}+{}".format(
                            sprite.tile * kBytesPer8x16Tile // segmentGranularity,
                            sprite.tile % ( segmentGranularity // kBytesPer8x16Tile ) )
                    else:
                        tile = "B_{}+{}".format(
                            sprite.tile*kBytesPer8x8Tile//segmentGranularity,
                            sprite.tile%(segmentGranularity//kBytesPer8x8Tile) )
...
def main():
    ...
    argParser.add_argument( "-g", "--segments", default="GRAPHICS",
        help="comma-separated list of output segments" )
    ...
    writeData( args.outprefix, allAnimationFrames, tiles,
               hardwareSpriteSize, segments, args.granularity )
