...
def packNesTile( tile ):
    ...
    return bitplane0All + bitplane1All
...
def writeNginData( nginCommonMapData, outPrefix, symbols ):
    with open( outPrefix + ".s", "w" ) as f:
        f.write( "; Data generated by tiled-map-importer.py\n\n" )
...
"""
        ...
        writeByteArray( f, "    attributes0: ",
            map( lambda x: x[ 4 ], nginCommonMapData.uniqueMt16 ) )
        f.write( ".endscope\n\n" )
...
"""
            ...
            #       set.
            kHeaderTemplate = """    ; ngin_MapData_Header
    .proc header
...
def main():
    ...
    argParser.add_argument( "-s", "--symbol", nargs="+" )
    argParser.add_argument( "-o", "--outprefix", required=True,
        help="prefix for output files/symbols" )
    ...
    if args.symbol is not None and len( args.infile ) != len( args.symbol ):
        raise Exception( "number of infiles must match the number of symbols" )
